...
% use UDS for convection for cflag=1
% use CDS for convection for cflag=2
% use CDS for diffusion in all cases

global nx ny awe aso ap ano aea q gfacx gfacy cflag dif mdotuf mdotvf dx dy

% First add CDS diffusion contributions
for i = 3:nx-2
  for j = 3:ny-2
    awe(i, j) = -dif * dy / dx;
    aso(i, j) = -dif * dx / dy;
    ap(i, j) = 2*dif * (dx/dy + dy/dx);
    ano(i, j) = -dif * dx / dy;
    aea(i, j) = -dif * dy / dx;
    q(i, j) = 0;
...
% Now use either UDS convection (cflag=1) or CDS convection (cflag=2)
if cflag == 1;  % UDS convection
  for i = 3:nx-2
    for j = 3:ny-2
      % Define face mass flow rates for convenience
      mdotw = - mdotuf(i-1, j);
      mdots = mdotvf(i, j-1);
      mdotn = - mdotvf(i, j);
      mdote = mdotuf(i, j);
      
      % In this flowfield, mdot_w is always into the CV (negative)
      awe(i, j) = awe(i, j) + mdotw;
      % In this flowfield, mdot_s is always out of the CV (positive)
      aso(i, j) = aso(i, j) + 0;
      ap(i, j) = ap(i, j) + mdots;
      % In this flowfield, mdot_n is always into the CV (negative)
      ano(i, j) = ano(i, j) + mdotn;
      % In this flowfield, mdot_e is always out of the CV (positive)
      aea(i, j) = aea(i, j) + mdote;
      % All terms include a Phi, so nothing goes here
      q(i, j) = q(i, j) + 0;
...
if cflag == 2;  % CDS convection
  for i = 3:nx-2
    for j = 3:ny-2
      % Define some face mass flow rates for convenience
      mdotw = - mdotuf(i-1, j);
      mdots = mdotvf(i, j-1);
      mdotn = - mdotvf(i, j);
      mdote = mdotuf(i, j);
      
      awe(j, i) = awe(i, j) + mdotw*gfacx(i);
      aso(i, j) = aso(i, j) + mdots*gfacy(j);
      ap(i, j) = ap(i, j) + mdotw*(1-gfacx(i)) + mdots*(1-gfacy(j)) + ...
                 mdotn*(1-gfacy(j+1)) + mdote*(1-gfacx(i+1));
      ano(i, j) = ano(i, j) + mdotn*gfacy(j+1);
      aea(i, j) = aea(i, j) + mdote*gfacx(i+1);
      q(i, j) = 0;
...
% Start at first internal node (i = 2)
i = 2;
...
awe(i, j) = 0;
aso(i, j) = 0;
ap(i, j) = 2*dif * (dx/dy + dy/dx) - dif * dx / dy;
ano(i, j) = -dif * dx / dy;
aea(i, j) = -dif * dy / dx;
q(i, j) = (1-yc(j))* dif * dy / dx;

% Loop through cells adjacent to left boundary
for j = 3:ny-2;
  % First add CDS diffusion contributions
  awe(i, j) = 0;
  aso(i, j) = - dif * dx / dy;
  ...
  aea(i, j) = -dif * dy / dx;
  q(i, j) = (1-yc(j))* dif * dy / dx;
  % Then add convection contributions depending on cflag
  % (not done)
end
...
% Now go along cells adjacent to the bottom boundary (j = 2)
j = 2
for i = 3:nx-2
  awe(i, j) = 0;
  aso(i, j) = 0;
...
% Now go along cells adjacent to the bottom boundary (j = ny-1)
% do diffusion contributions
% do convection contributions

% Now go along cells adjacent to the right boundary (i = nx-1)
% do diffusion contributions
% do convection contributions
