...
% Solve for phi with iterative solversphi
numiters = []
cpus = []
mdottots = []
phidottots = []
...
jacobi;
cpus(end+1) = toc
phi = phinew; bndryflowrates;
numiters(end+1) = iterstore
mdottots(end+1) = mdot_tot
phidottots(end+1) = phidot_tot

figure; 
plot(1:8000, resmax);
xlabel('Iteration');
ylabel('Maximum Error Magnitude');
title('Jacobi');
...
for omegas = 1.0:0.1:2.0
  ...
  plot(1:8000, resmax);
  xlabel('Iteration');
  ylabel('Residual Error Magnitude');
  title(sprintf('GS_SOR, omega = %1.1f', omega));
end

for ii = 1:length(cpus)
  fprintf('%.0f,    %.8e,    %.8e,     %.8e\n', numiters(ii), cpus(ii), ...
          mdottots(ii), phidottots(ii))
end