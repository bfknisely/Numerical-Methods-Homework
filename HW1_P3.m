...
loglog(h, abs(bds1(:, 2)), 'ko', h, abs(cds2(:, 2)), 'rx',...
    h, abs(fds3(:, 2)), 'b*', h, abs(bds3(:, 2)), 'ms', ...
    h, abs(cds4(:, 2)), 'kp'); 
xlabel('Spacing \it{h}'); ylabel('|Error|'); 
legend('BDS1', 'CDS2', 'FDS3', 'BDS3', 'CDS4', 'location', 'southwest'); 
set(gca, 'fontsize', 16);
xlim([1e-13, 10]);
...
% Print error results
fprintf(fid,'%-2s|%-12s|%-12s|%-12s|%-12s|%-12s|%-12s\n', ...
    'i', '      h', ' Error:BDS1', ' Error:CDS2', ' Error:FDS3', ...
    ' Error:BDS3', ' Error:CDS4');
for i = 1:n
    fprintf(fid,'%2.0f,%12.5e,%12.5e,%12.5e,%12.5e,%12.5e,%12.5e,%12.5e', ...
    i, h(i), bds1(i, 2), cds2(i, 2), fds3(i, 2), bds3(i, 2), cds4(i, 2));
    fprintf(fid,'\n');
end
